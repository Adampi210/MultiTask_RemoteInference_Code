"""
MAF1_probabilistic.py - probabilistic-transmission MAF (Max-Age-First).

Priority defaults to current AoI (= MAF). reliability_aware=True scales by q so
that less reliable links get lower priority for the same age.

AoI evolution after greedy selection is stochastic: scheduled pair resets only
on Bernoulli(q[m,j]) success; otherwise it ages by 1.
"""
import os
import sys
import numpy as np
...
def _single_rollout(M, T, K, B, km, n, c_use, N, w, p, gamma, q, rng,
    ...
    for t in range(K):
        age = Delta.astype(float) + 1.0
        priority = q * age if reliability_aware else age
...
def MAF1_probabilistic(
    ...
    """Probabilistic MAF. Default priority = Delta + 1 (pure MAF)."""
    ...
    if mc_trials == 1:
        out = float(results[0])
        if verbose:
            label = "MAF1_p(rel)" if reliability_aware else "MAF1_p"
