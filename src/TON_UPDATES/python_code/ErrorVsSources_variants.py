...
recommended_subgradient_methods.json unless INFOCOM_SUBGRADIENT_METHOD is set.
...
Outputs:
    ErrorVsSources_variants_data.csv
    ErrorVsSources_variants.png
"""
import os
import sys
import csv
...
def main():
    ...
    csv_path = os.path.join(here, "ErrorVsSources_variants_data.csv")
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["M", "MGF", "MAF", "MIEF", "Random",
                    "subgradient_method"])
        for i, M in enumerate(SOURCES):
            w.writerow([M, pmgf[i], pmaf[i], pmief[i], prand[i],
                        SUBGRAD_METHOD])
    ...
    out = os.path.join(here, "ErrorVsSources_variants.png")
