...
def main():
    np.random.seed(SEED)
    M_default = 40  # unused below but matches MATLAB's first M=40 declaration
    N = 10
    km = 9
    B = 20
    T = 100
    ...
    for j in range(1, km + 1):
        ...
        leftover_j = j  # final value of j in MATLAB after the loop

    sources = np.arange(3, 22, 3)  # MATLAB 3:3:21
    ...
    for ch in range(len(sources)):
        ...
        # Faithful: only the column j = leftover_j is set; the rest stay 0.
        w = np.zeros((M, km))
        j_idx = leftover_j - 1  # 0-indexed
        for m in range(M):
            if (m + 1) > 0.5 * M:
                w[m, j_idx] = 0.01
            else:
    ...
    np.savetxt(os.path.join(here, 'ErrorVsSourcesM_data.csv'),
    ...
    out = os.path.join(here, 'ErrorVsSourcesM.png')
