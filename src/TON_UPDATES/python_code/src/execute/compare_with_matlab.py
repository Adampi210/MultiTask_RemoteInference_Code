...
# Side-by-side _compare PNGs live next to the rest of the deterministic plots.
DATA_DIR = paths.DATA_DETERMINISTIC_DIR
PLOT_DIR = paths.PLOTS_DETERMINISTIC_DIR
# MATLAB .fig and .jpg references now live in matlab_code/.
ROOT = paths.MATLAB_DIR
...
def _compare_policy_plot(py_csv, fig_file, title, xlabel, ylabel,
    ...
    py_path = os.path.join(DATA_DIR, py_csv)
...
def _emit_python_only_compare(py_png, title, note, out_name):
    ...
    py_path = os.path.join(PLOT_DIR, py_png)
    if not os.path.isfile(py_path):
