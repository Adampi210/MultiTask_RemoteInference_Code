...
DATA_DIR = paths.DATA_PROBABILISTIC_DIR
PLOT_DIR = paths.PLOTS_PROBABILISTIC_DIR
...
def main():
    ...
    for M in M_VALS:
        ...
        for profile in PROFILES:
            ...
            for method in methods:
                ...
                # subgradientiter1_probabilistic emits these history keys:
                # max_lambda / max_mu / mean_compute_positive_residual /
                # mean_channel_positive_residual.
                tail_lam = _tail_stability(hist['max_lambda'])
                tail_mu = _tail_stability(hist['max_mu'])
                avg_comp = float(np.mean(hist['mean_compute_positive_residual']))
                avg_chan = float(np.mean(hist['mean_channel_positive_residual']))

                print(f"  {method:>11s}: obj={mean_obj:.5g}+-{std_obj:.3g}  "
                      f"runtime={training_runtime:.2f}s  "
                      f"|lam|max={hist['max_lambda'][-1]:.3f}  "
                      f"|mu|max={hist['max_mu'][-1]:.3f}  "
                ...
                rows.append({
                    ...
                    "final_lambda_max": hist['max_lambda'][-1],
                    "final_mu_max": hist['max_mu'][-1],
    ...
    # --- CSV ---
    csv_path = os.path.join(DATA_DIR, "CompareSubgradient_probabilistic_data.csv")
    ...
    plt.tight_layout()
    plot_path = os.path.join(PLOT_DIR, "CompareSubgradient_probabilistic.png")
